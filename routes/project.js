...
const upload = multer({ 
  ...
  fileFilter: (req, file, callback) => {
    ...
    if (file.size > maxSizeLargeImage) {
      ...
      if (projectImagesData && projectImagesData.length > 0) {
        ...
        const updateProjectImages = newImages.map( newImage => {
          ...
          for (const update of req.body.updateProjectImages) {
            if (update.listIdx == newImage.listIdx && update.idx == newImage.idx) {
        ...
        updateProjectImages.forEach( update => {
          project.imageList[update.listIdx].images[idx].imageURL = update.imageURL
          project.imageList[update.listIdx].images[idx].imagePublicId = update.imagePublicId
        })
      }
